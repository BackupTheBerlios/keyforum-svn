...
sub new {
	...
	$this->{Congi}=$db->prepare("INSERT INTO ".$fname."_congi (`HASH`,`TYPE`,`WRITE_DATE`,`INSTIME`) VALUES(?,?,?,?);");
...
sub _AddType2_edit {
	...
	$this->{Congi}->execute($md5,"2",$msg->{DATE},time());
...
sub _AddType1_edit {
	...
	my $cambiati='0';
	$cambiati='1' if $this->{DB}->{mysql_info}=~/Changed: (\d+?)/ && $1;
	$this->{Congi}->execute($md5,"1",$msg->{DATE},time());
...
sub _AddType4 {
	...
	$this->{Congi}->execute($md5,"4",$msg->{DATE},time()) unless $this->{UpDateCongi}->rows;
...
sub _UpDateType4 {
	...
	$msg->{PKEY}=ConvData::Dec2Bin($msg->{PKEY});
	$this->{Congi}->execute($md5,"4",$msg->{DATE},time());
	$this->{UpdateMember}->execute($msg->{AUTORE},$msg->{DATE},$msg->{PKEY},$msg->{SIGN},$md5);
	return 1;
}
