...
sub ConfTable {
	...
	my $fname=$this->{Fname};
	my $last_edit;
	my $sth=$this->{DB}->prepare("SELECT `VALUE` FROM ".$fname."_conf WHERE `GROUP`='ADMIN' AND `FKEY`='LAST_EDIT'");
	$sth->execute;
	...
	$sth=$this->{DB}->prepare("INSERT INTO ".$fname."_conf (`GROUP`,`FKEY`,`VALUE`) VALUE(?,?,?)");
	$sth->execute('ADMIN','LAST_EDIT',$date);
	$sth->execute($_->{a} || '',$_->{b} || '',$_->{c} || '') while $_=pop(@$com);
	return 1;
