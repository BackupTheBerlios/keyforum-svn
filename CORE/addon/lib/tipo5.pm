...
my @campi=('DATE','TYPE','AUTORE','FNAME','BODY','EXTVAR'); # Questi sono i campi che verrano usati per il calcolo dell'md5
...
sub SelectQuery {
    ...
    my $tosend=[];
    push(@$tosend,
        "SELECT `HASH`,  AUTORE`, `FNAME`, '$thistype' AS `TYPE`,`DATE`,`FNAME`, `BODY`, `EXTVAR`, `SIGN`,`ADMIN_SIGN` FROM ".$this->{fname}."_files WHERE `HASH`=?;");
