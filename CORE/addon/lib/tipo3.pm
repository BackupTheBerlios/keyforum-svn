...
sub SelectQuery {
    ...
    my $tosend=[];
    push(@$tosend,
        "SELECT `HASH`, `SEZ`, `AUTORE`, `EDIT_OF`, '$thistype' AS `TYPE`,`DATE`,`IS_EDIT`, `TITLE`, `SUBTITLE`, `BODY`, `EXTVAR`, `SIGN`,`FOR_SIGN`,`ADMIN_SIGN` FROM ".$this->{fname}."_newmsg WHERE `HASH`=?;",
        "SELECT `HASH`, `SEZ`, `AUTORE`, `EDIT_OF`, '$thistype' AS `TYPE`,`DATE`,`IS_EDIT`, `TITLE`, `SUBTITLE`, `BODY`, `EXTVAR`, `SIGN`,`FOR_SIGN`,`ADMIN_SIGN` FROM ".$this->{fname}."_newmsg WHERE `EDIT_OF`=? AND `IS_EDIT`='1' LIMIT 100;",
...
sub new {
    ...
    $this->{Inserisci}=$GLOBAL::SQL->prepare("INSERT INTO ".$fname."_newmsg (`HASH`,`SEZ`,`AUTORE`, `EDIT_OF`,`DATE`,`IS_EDIT`, `TITLE`, `SUBTITLE`, `BODY`, `EXTVAR`, `SIGN`,`FOR_SIGN`,`ADMIN_SIGN`,`visibile`) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
...
sub Inserisci {
    ...
    if (length($sez_data->{PKEY})>100) { # se la sezione è protetta da password
        $msg->{ERRORE}=29, return undef if length($msg->{FOR_SIGN})<100;  # 29 il forum è protetto da password e il messaggio non è firmato
        $msg->{ERRORE}=30, return undef unless $futils->CheckSignPkey($msg->{TRUEMD5},$msg->{FOR_SIGN},$sez_data->{PKEY}); #30 Firma del forum non valida
    }
...
sub _inserisci {
    ...
    $this->{Inserisci}->execute($msg->{TRUEMD5},$msg->{SEZ},$msg->{AUTORE},$msg->{EDIT_OF},$msg->{DATE},$msg->{IS_EDIT},
        $msg->{TITLE} || '',$msg->{SUBTITLE} || '',$msg->{BODY},$msg->{EXTVAR} || '',$msg->{SIGN},$msg->{FOR_SIGN} || '',$msg->{ADMIN_SIGN} || '',$cambiati);
