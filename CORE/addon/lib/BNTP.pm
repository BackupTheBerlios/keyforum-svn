...
sub get_ntp_response{
    ...
    $sock = IO::Socket::INET->new(
        Proto    => 'udp',
        PeerHost => $host,
        PeerPort => $port ) or return Errore("Errore nel modulo BNTP.pm :".$@."\n");
	#return Errore($@) if $@;
    ...
    #print "--->$client_localtime $client_frac_localtime ".NTP_ADJ."\n";
    my $ntp_msg =
      pack( "B8 C3 N10 B32", '00011011', (0) x 12, int($client_localtime),
      $client_frac_localtime );

    $sock->send($ntp_msg) or return Errore("BNTP.pm: send() failed: $!\n");
	...
	vec($rin,fileno($sock),1) = 1;
	 (select($rin,undef,undef,2)) ? # Il timeout è di due secondi
		($sock->recv($data,960) or return undef) :
		(return Errore("Collimazione orologio con $host fallita. Timeout superato.\n"));
	return undef if length($data)<40;
	#alarm(0)
   # };
