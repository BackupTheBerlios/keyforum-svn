...
if ( !empty($nick) and !empty($password) and empty($privkey) ) { // create a new user
	...
	$sqladd = "INSERT INTO $SNAME" . "_localmember (hash, password) VALUES ('"
                    . $identif . "','" . mysql_real_escape_string($rsapriv) . "')";
        if ( !mysql_query($sqladd) ) die("".mysql_error()."");
        else echo "Ok<br><br>";
	...
	$addreq['FORUM']['ADDMSG']['DATE'] = $coreresp['CORE']['INFO']['GMT_TIME'];
	$addreq['FORUM']['ADDMSG']['PKEYDEC'] = $rsapub;
	$addreq['FORUM']['ADDMSG']['_PRIVATE'] = base64_decode($rsapriv);
	$addreq['FORUM']['ADDMSG']['_PWD'] = md5($nick . md5($password,TRUE),TRUE);
	#$addreq['FORUM']['ADDMSG']['SIGN'] = $firma_rsa;
	$addreq['FORUM']['ADDMSG']['TYPE'] = '2';
	$addreq['FORUM']['ADDMSG']['FDEST'] = pack("H*",$idforum);
	
	if ( !$coresk->Send($addreq) ) die("Error sending the request to the core!");
	$coreresp = $coresk->Read();
	print "errore:".$coreresp['FORUM']['ADDMSG'][ERRORE]."<br>\n";
	if ( !$coreresp ) die("Error receiving response form the core!");
	if ( $coreresp['FORUM']['ADDMSG'][ERRORE] == -2 ) die("Forum unknown, cannot register the user.");
	if ( $coreresp['FORUM']['ADDMSG'][ERRORE] == -1 ) die("The Core didn't accept the message, aborting.");
	if (! $coreresp['FORUM']['ADDMSG'][ERRORE] ) echo "Ok<br><br>";
	include("end.php");
	exit(0);			
}
